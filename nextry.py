...
class Parser:
    ...
    def term(self):
        node=self.factor()
        while self.current_token.type is not None and self.current_token.type in ('MUL', 'DIV'):
            if(self.current_token.type=='DIV' or self.current_token.type=='MUL'):
                token= self.current_token
                self.next_token()
    ...
    def ifexprs(self):
        ...
        if self.current_token.type == IF:  
            ...
            while self.current_token.type == SEMI and self.peek_next_token().type != RBRACES:
                self.next_token()
                expressions.append(self.parse_var_decl())
            if self.current_token.type != SEMI:
                raise Exception("Expected semicolon")
            self.next_token()
            if self.current_token.type != RBRACES:
